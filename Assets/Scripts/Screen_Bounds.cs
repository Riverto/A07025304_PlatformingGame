...
public class Screen_Bounds : MonoBehaviour {
    ...
    public float buffer = 1.0f; // set this so the sprite disappears offscreen before re-appearing on other side
    public float distanceZ = 10.0f;
    private Rigidbody2D playerBody;

    void Awake()
    {
        // set Vector3 to ( camera left/right limits, sprite Y, sprite Z )
    ...
    void Update()
    {
        ...
        if (playerBody.position.x < leftConstraint - buffer) { // sprite is past world-space view / off screen
            playerBody.position = new Vector2(rightConstraint, playerBody.position.y);  // move sprite to opposite side
        }

        if (playerBody.position.x > rightConstraint + buffer){ // sprite is past world-space view / off screen
            playerBody.position = new Vector2(leftConstraint, playerBody.position.y);   // move sprite to opposite side
        }
    }
}
